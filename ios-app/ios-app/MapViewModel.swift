...
class MapViewModel: NSObject, ObservableObject {
    ...
    /// Built-in NYC area hospitals — used when API fails (e.g. simulator network restrictions)
    ...
    private func fetchHospitals() async {
        ...
        if let fetched = await fetchHospitalsFromOpenDataSoft(), !fetched.isEmpty {
            ...
            // 3. Built-in list when both APIs fail
            let center = userLocation ?? Self.columbiaUniversity
    ...
    /// Ensures Mt. Sinai Morningside (next to Columbia) is always in the list
    ...
    /// OpenDataSoft: NYC boroughs filter (New York, Kings, Queens, Bronx, Richmond) — ~70 hospitals
    ...
    /// Overpass API (OpenStreetMap): hospitals in NYC bounding box
    ...
    /// Overpass API: pharmacies in NYC bounding box (tightened scope, limit 250, 15s timeout)
